...
public class Emprestimus implements EmprestimusIF {
	...
	public String cadastrarItem(String idSessao, String nome, String descricao,
		...
		assertStringNaoVazia(categoria,
				...
				Mensagem.CATEGORIA_INVALIDA.getMensagem());
		asserteTrue(autenticacao.existeIdSessao(idSessao),
	...
	public String localizarUsuario(String idSessao, String chave,
		...
		asserteTrue(autenticacao.existeIdSessao(idSessao),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		assertStringNaoVazia(chave,
				Mensagem.PALAVRA_CHAVE_INVALIDA.getMensagem(),
				Mensagem.PALAVRA_CHAVE_INVALIDA.getMensagem());
		...
		UsuarioIF usuario = autenticacao.getUsuarioPeloIDSessao(idSessao);
		RefCoordenadas coordenadas = new RefCoordenadas(usuario.getLatitude(),
				usuario.getLongitude());
		Collections.sort(users, new ComparaDistancia(coordenadas));
		String saida = "";
		for (int i = 0; i < users.size(); i++) {
			saida += users.get(i).getNome() + " - "
					+ users.get(i).getEndereco();
			if (i != users.size() - 1) {
				saida += "; ";
			}
		}
	...
	public void requisitarAmizade(String idSessao, String login)
			throws Exception {
		Validador.assertStringNaoVazia(idSessao,
				Mensagem.SESSAO_INVALIDA.getMensagem(),
				Mensagem.SESSAO_INVALIDA.getMensagem());
		Validador.assertStringNaoVazia(login,
				Mensagem.LOGIN_INVALIDO.getMensagem(),
				Mensagem.LOGIN_INVALIDO.getMensagem());
		Validador.asserteTrue(autenticacao.existeIdSessao(idSessao),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		Validador.asserteTrue(Autenticacao.existeUsuario(login),
	...
	public void aprovarAmizade(String idSessao, String login) throws Exception {
		Validador.assertStringNaoVazia(idSessao,
				Mensagem.SESSAO_INVALIDA.getMensagem(),
				Mensagem.SESSAO_INVALIDA.getMensagem());
		Validador.asserteTrue(autenticacao.existeIdSessao(idSessao.trim()),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		Validador.assertStringNaoVazia(login,
				Mensagem.LOGIN_INVALIDO.getMensagem(),
				Mensagem.LOGIN_INVALIDO.getMensagem());
		Validador.asserteTrue(Autenticacao.existeUsuario(login.trim()),
				Mensagem.USUARIO_INEXISTENTE.getMensagem());
	...
	public String ehAmigo(String idSessao, String login) throws Exception {
		...
		asserteTrue(autenticacao.existeIdSessao(idSessao),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		asserteTrue(Autenticacao.existeUsuario(login.trim()),
				Mensagem.USUARIO_INEXISTENTE.getMensagem());
	...
	public String getItens(String idSessao, String login) throws Exception {
		...
		Validador.asserteTrue(Autenticacao.existeUsuario(login),
				Mensagem.USUARIO_INEXISTENTE.getMensagem());
		Validador.asserteTrue(autenticacao.getUsuarioPeloIDSessao(idSessao)
				.ehAmigo(login), Mensagem.USUARIO_NAO_TEM_PEMISSAO_VER_ITENS
				.getMensagem());
	 ...
	 */
	@Override
	public synchronized String requisitarEmprestimo(String idSessao,
		...
		Validador.assertStringNaoVazia(idItem,
				Mensagem.ID_ITEM_INVALIDO.getMensagem(),
				Mensagem.ID_ITEM_INVALIDO.getMensagem());
		Validador.asserteTrue(ItemRepositorio.existeItem(idItem),
				Mensagem.ID_ITEM_INEXISTENTE.getMensagem());
	...
	public String aprovarEmprestimo(String idSessao,
		...
		assertStringNaoVazia(idSessao, Mensagem.SESSAO_INVALIDA.getMensagem(),
				Mensagem.SESSAO_INVALIDA.getMensagem());
		asserteTrue(autenticacao.existeIdSessao(idSessao.trim()),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		...
		UsuarioIF usuario = autenticacao
				.getUsuarioPeloIDSessao(idSessao.trim());
	...
	public synchronized void devolverItem(String idSessao, String idEmprestimo)
		...
		asserteTrue(autenticacao.existeIdSessao(idSessao),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		assertStringNaoVazia(idEmprestimo,
		...
		UsuarioIF usuario = autenticacao.getUsuarioPeloIDSessao(idSessao);
		EmprestimoIF emp = EmprestimoRepositorio
				.recuperarEmprestimo(idEmprestimo);
		asserteTrue(emp.getBeneficiado().equals(usuario),
				Mensagem.EMPRESTIMO_DEVOLUCAO_APENAS_BENEFICIADO.getMensagem());
	...
	@Override
	public synchronized void requisitarDevolucao(String idSessao,
			String idEmprestimo) throws Exception {
		...
		String assunto = "Empréstimo do item " + emprestimo.getItem().getNome()
				+ " a " + amigo.getNome() + "";
	...
	public void confirmarTerminoEmprestimo(String idSessao, String idEmprestimo)
		...
		asserteTrue(emp.getEmprestador().equals(usuario),
				Mensagem.EMPRESTIMO_DEVOLUCAO_CONFIRMADA_APENAS_EMPRESTADOR
						.getMensagem());
	 ...
	 * @throws Exception
	 */
	private void liberaItem(String idSessao, String idEmprestimo)
			throws Exception {
		EmprestimoIF emprestimo = EmprestimoRepositorio
				.recuperarEmprestimo(idEmprestimo);
		...
		for (UsuarioIF interessado : item.getInteresasados()) {
			enviarMensagem(
					idSessao,
					interessado.getLogin(),
					"O item " + item.getNome() + " do usuário "
							+ dono.getNome() + " está disponível",
	...
	public String enviarMensagem(String idSessao, String destinatario,
		...
		asserteTrue(
				EmprestimoRepositorio.existeEmprestimo(idRequisicaoEmprestimo
						.trim()),
				Mensagem.ID_REQUISICAO_EMP_INEXISTENTE.getMensagem());
		return usuario.enviarMensagemEmprestimo(destinatario, assunto,
				mensagem, idRequisicaoEmprestimo);
	}

	@Override
	public synchronized String lerTopicos(String idSessao, String tipo)
			throws Exception {
		assertStringNaoVazia(idSessao, Mensagem.SESSAO_INVALIDA.getMensagem(),
				Mensagem.SESSAO_INVALIDA.getMensagem());
		asserteTrue(autenticacao.existeIdSessao(idSessao),
	...
	public String lerMensagens(String idSessao, String idTopico)
		...
		UsuarioIF usuario = autenticacao.getUsuarioPeloIDSessao(idSessao);
		if (!usuario.equals(conversa.getRemetente())
				&& !usuario.equals(conversa.getDestinatario()))
			throw new Exception(
					Mensagem.USUARIO_SEM_PERMISSAO_LEITURA_TOPICO.getMensagem());
	...
	public String pesquisarItem(String idSessao, String chave, String atributo,
		...
		assertStringNaoVazia(tipoOrdenacao,
				Mensagem.ORDENACAO_TIPO_INVALIDO.getMensagem(),
				Mensagem.ORDENACAO_TIPO_INVALIDO.getMensagem());
		if (!tipoOrdenacao.trim().equalsIgnoreCase("crescente")
				&& !tipoOrdenacao.trim().equalsIgnoreCase("decrescente")) {
			throw new Exception(
					Mensagem.ORDENACAO_TIPO_INEXISTENTE.getMensagem());

		}

		assertStringNaoVazia(criterioOrdenacao,
				Mensagem.ORDENACAO_CRITERIO_INVALIDO.getMensagem(),
				Mensagem.ORDENACAO_CRITERIO_INVALIDO.getMensagem());
		if (!criterioOrdenacao.trim().equalsIgnoreCase("dataCriacao")
				&& !criterioOrdenacao.trim().equalsIgnoreCase("reputacao")) {
			throw new Exception(
					Mensagem.ORDENACAO_CRITERIO_INEXISTENTE.getMensagem());

		}

		UsuarioIF usuario = autenticacao.getUsuarioPeloIDSessao(idSessao);

		return usuario.pesquisarItem(chave, atributo, tipoOrdenacao,
				criterioOrdenacao);
	}
	...
	public void apagarItem(String idSessao, String idItem) throws Exception {
		...
		asserteTrue(item.estahDisponivel(),
				Mensagem.NAO_PODE_EMPRESTAR_ITEM_EMPRESTADO.getMensagem());
	...
	public String getRanking(String idSessao, String categoria)
		...
		if (categoria.trim().equalsIgnoreCase("global")) {
			Iterator<UsuarioIF> iteradorColecaoUsuarios = autenticacao
					.getListaUsuarios().iterator();
		...
		if (saidaRanking.toString().trim().equals(""))
			return "";
		return saidaRanking.toString().trim()
				.substring(0, saidaRanking.toString().trim().length() - 1);
	}
	...
	public String historicoAtividadesConjunto(String idSessao) throws Exception {
		return GerenciadorDeNotificacoes.getInstance()
				.getHistoricoAtividadesConjunto(
	...
	public String publicarPedido(String idSessao, String nomeItem,
		...
		asserteTrue(autenticacao.existeIdSessao(idSessao),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		assertStringNaoVazia(nomeItem, Mensagem.NOME_INVALIDO.getMensagem(),
				Mensagem.NOME_INVALIDO.getMensagem());
		assertStringNaoVazia(descricaoItem,
				Mensagem.DESCRICAO_INVALIDA.getMensagem(),
				Mensagem.DESCRICAO_INVALIDA.getMensagem());
		return autenticacao.getUsuarioPeloIDSessao(idSessao).publicarPedido(
	...
	public void oferecerItem(String idSessao, String idPublicacaoPedido,
		...
		autenticacao.getUsuarioPeloIDSessao(idSessao).oferecerItem(
				idPublicacaoPedido, idItem);
	...
	public void republicarPedido(String idSessao, String idPublicacaoPedido)
		...
		asserteTrue(autenticacao.existeIdSessao(idSessao),
				Mensagem.SESSAO_INEXISTENTE.getMensagem());
		autenticacao.getUsuarioPeloIDSessao(idSessao).republicarPedido(
				idPublicacaoPedido);
