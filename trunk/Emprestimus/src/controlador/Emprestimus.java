...
public class Emprestimus implements EmprestimusIF {
	...
	private static Emprestimus instanciaUnica = null;
	...
	private static String[] nome_invalido = { "Nome inválido",
			"Nome inválido", "Nome inválido" };
	private static String[] login_invalido = { "Login inválido",
			"Login inválido", "Login inválido", "Login inválido" };
	private static String[] endereco_invalido = { "Endereco inválido",
			"Endereco inválido", "Endereco inválido" };
	private static String[] atributo_invalido = { "Atributo inválido",
			"Atributo inválido", "Atributo inválido", "Atributo inválido" };

	private Emprestimus() {
	}

	/**
	 * Recupera a instancia de Autenticacao. Nesta situacao, temos a intencao de
	 * evitar a despesa de pegar o bloqueio da classe Singleton cada vez que o
	 * metodo eh chamado. O bloqueio eh agarrado somente se a instancia
	 * Singleton nao existe, e em seguida a existencia da ocorrencia eh
	 * verificada novamente no caso de outro segmento passar o primeiro cheque
	 * um instante antes de o segmento atual.
	 * 
	 * @return A instancia de RelacionamentoAmigos.
	 */
	public static synchronized Emprestimus getInstance() {
		if (instanciaUnica == null) {
			synchronized (Emprestimus.class) {
				if (instanciaUnica == null) {
					instanciaUnica = new Emprestimus();
					inicializaAtributos();
				}
			}
		}
	...
	public void criarUsuario(String nome, String login, String endereco)
		...
		if (usuariosCadastrados.contains(usuario)) // Assegura que nao existirao
	...
	public Long abrirSessao(String login) throws IllegalArgumentException {
		Usuario user = getUsuario(ValidadorString.pegaLogin(login_invalido,
				login));
		if (user != null) {
			Long idSessao = geraIDSessao();
	...
	public void zerarSistema() {
		inicializaAtributos();
		// TODO Apagar depois: return (usuariosCadastrados.size() == 0 &&
		// sessoes.size() == 0);
	}
